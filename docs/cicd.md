...
This mirrors the environment tiers and deployment history described by [GitLab Environments](https://docs.gitlab.com/ci/environments/) while keeping the flow small enough for a personal server.

## Pull request previews

Managed previews require a Coolify application in API mode and a full URL template containing `{{pr_id}}`, for example `https://pr-{{pr_id}}.preview.example.com`. Create wildcard DNS before enabling the feature. DevPilot's generated GitHub Actions and GitLab CI workflows build a commit-addressed `sha-*` image after quality and security gates, then call the dedicated signed Preview endpoint. DevPilot asks Coolify to create the isolated runtime with its documented [`pull_request_id` and `docker_tag` parameters](https://next.coolify.io/docs/api/endpoints/deployments/deploy-by-tag-or-uuid), follows Provider deployment state, and exposes the Preview URL in the workspace.

Closing the PR/MR calls Coolify's dedicated [Preview deletion API](https://next.coolify.io/docs/api/endpoints/applications/delete-preview-deployment-by-pull-request-id). A durable TTL is an independent safety net: expired environments are deleted even if the CI close job never ran, and transient cleanup failures retry after 15 minutes. Application deletion and Provider credential changes are blocked while a tracked Preview remains active, preventing orphan containers.

Preview callbacks use a separate one-time HMAC secret that cannot authenticate production release evidence. Generated workflows allow only branches from the same repository/project; fork previews are intentionally skipped. Coolify Preview Environment Variables must contain non-production credentials only. This follows Coolify's [Preview Deployments](https://next.coolify.io/docs/applications/deployments/preview-deployments) isolation guidance and GitLab's [Review Apps](https://docs.gitlab.com/ci/review_apps/) lifecycle model.
