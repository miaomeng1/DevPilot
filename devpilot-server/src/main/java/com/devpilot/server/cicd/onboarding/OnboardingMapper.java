...
public interface OnboardingMapper extends BaseMapper<OnboardingJob> {
    ...
    @Update("""
        ...
        WHERE id = #{id} AND stage < 5 AND request_cipher IS NOT NULL AND status <> 'EXPIRED'
    ...
    @Select("""
        SELECT * FROM cicd_onboarding WHERE request_cipher IS NOT NULL AND status <> 'EXPIRED'
          AND credentials_updated_at < #{cutoff} AND (lease_until IS NULL OR lease_until < #{now})
        """)
    java.util.List<OnboardingJob> expiring(@Param("cutoff") LocalDateTime cutoff, @Param("now") LocalDateTime now);
}
