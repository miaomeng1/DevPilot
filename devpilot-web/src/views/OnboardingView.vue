...
import { beginApplicationCreation, completeApplicationCreation, readApplicationCreation, type PendingApplicationCreation } from '@/utils/applicationCreationRequest'
...
interface Job { id: string; applicationId: string; stage: number; status: string; resourceId: string | null; changeUrl: string | null; errorMessage: string | null; containerPort: number | null; hostPort: number | null; healthPath: string | null; environmentVariableNames: string[]; registryCredentialsRequired: boolean }
const route = useRoute(), router = useRouter(), servers = useServerStore()
const auth = useAuthStore(), pendingApplication = ref<PendingApplicationCreation | null>(null)
...
const platform = ref<{ providerUrl: string | null; providerTokenConfigured: boolean; revision: string; publicUrl: string; serverId: string | null } | null>(null)
let mounted = true
const form = reactive({
  ...
  usePlatformConnection: false, platformRevision: '',
})
function selectPlatformConnection() {
  inspection.value = null; confirmed.value = false
  if (form.usePlatformConnection && platform.value) {
    form.deploymentProvider = 'DOKPLOY'; form.providerBaseUrl = platform.value.providerUrl || ''
    form.providerApiToken = ''; form.platformRevision = platform.value.revision
    form.publicBaseUrl = platform.value.publicUrl
    if (!form.serverId && platform.value.serverId) form.serverId = platform.value.serverId
  }
}
...
async function start() {
  ...
  try {
    ...
    if (!applicationId.value) {
      pendingApplication.value = beginApplicationCreation(auth.user?.id, { name: form.name, code: form.code, serverId: form.serverId,
      ...
      const app = await applicationApi.create({ ...pendingApplication.value.payload, requestId: pendingApplication.value.requestId })
      applicationId.value = app.id
      // Persist the application reference before task creation: a failed POST must not orphan it on refresh.
      await router.replace({ query: { applicationId: applicationId.value } })
      completeApplicationCreation(auth.user?.id, pendingApplication.value.requestId); pendingApplication.value = null
    }
...
async function refresh() {
  try {
    job.value = (await apiClient.get<ApiResponse<Job>>(`/cicd/onboarding/${applicationId.value}`)).data.data
    if (job.value.containerPort !== null) form.containerPort = job.value.containerPort
    if (job.value.hostPort !== null) form.hostPort = job.value.hostPort
    if (job.value.healthPath !== null) form.healthPath = job.value.healthPath
  }
...
async function updateCredentials() {
  ...
  try {
    job.value = (await apiClient.put<ApiResponse<Job>>(`/cicd/onboarding/${applicationId.value}/credentials`, {
      ...
      environmentValues: job.value?.status === 'EXPIRED' ? variables() : undefined,
      providerQuotaConfirmed: form.providerQuotaConfirmed,
    })).data.data
...
async function updatePorts() {
  busy.value = true; error.value = ''
  try {
    job.value = (await apiClient.put<ApiResponse<Job>>(`/cicd/onboarding/${applicationId.value}/ports`, {
      containerPort: form.containerPort, hostPort: form.hostPort, healthPath: form.healthPath,
    })).data.data
  } catch (cause) { error.value = apiErrorMessage(cause, '无法修改端口；请刷新确认任务是否已进入资源创建阶段') }
  finally { busy.value = false }
}
onMounted(async () => {
  if (typeof route.query.applicationId !== 'string') {
    try {
      pendingApplication.value = readApplicationCreation(auth.user?.id)
      if (pendingApplication.value) {
        const saved = pendingApplication.value.payload, health = new URL(saved.healthCheckUrl)
        Object.assign(form, { name: saved.name, code: saved.code, serverId: saved.serverId,
          hostPort: Number(health.port), healthPath: health.pathname })
      }
    } catch { error.value = '无法恢复原应用创建标识，请核对已有应用并保留浏览器记录，不能重新创建' }
  }
  try { platform.value = (await apiClient.get<ApiResponse<NonNullable<typeof platform.value>>>('/setup')).data.data }
  catch { /* Manual connection remains available when setup has not been loaded. */ }
...
<template>
  <main class="onboard-page">
    ...
    <p v-if="pendingApplication && !job" role="status" class="verification-note">有尚未完成的应用创建请求：{{ pendingApplication.payload.name }}（{{ pendingApplication.payload.code }}）。已恢复原名称、编码、服务器和健康地址；请重新授权并核对同一项目。重试会复用原请求，不会自动创建第二个应用。浏览器不保存仓库 Token 或业务环境变量。</p>
    <section v-if="job" class="card progress" aria-live="polite">
      ...
      <details v-if="job.stage === 0 && job.status === 'FAILED'"><summary>端口冲突？修改预检参数</summary>
        <p>只修改此任务的预检参数，不创建资源、不部署。保存后点击继续重新检查；请核对全部三个值。</p>
        <div class="grid">
          <label>容器端口 Container port<input v-model.number="form.containerPort" type="number" min="1" max="65535" /></label>
          <label>发布端口 Host port<input v-model.number="form.hostPort" type="number" min="1024" max="65535" /></label>
          <label>健康检查路径<input v-model="form.healthPath" /></label>
        </div>
        <button :disabled="busy" @click="updatePorts">保存参数，不执行部署</button>
      </details>
      <details v-if="job.status === 'FAILED' || job.status === 'EXPIRED'"><summary>{{ job.status === 'EXPIRED' ? '重新授权，恢复原任务' : '授权失效？更新凭据（留空表示不修改）' }}</summary>
        <div v-if="job.status === 'EXPIRED'">
          <p>仓库 Token、部署平台 Key 必填。{{ job.registryCredentialsRequired ? '还需重新提供 Registry 密码。' : '' }}保留原应用及阶段，不创建新任务。</p>
          <label v-if="job.environmentVariableNames.length">重新填写环境变量：{{ job.environmentVariableNames.join(', ') }}<textarea v-model="envText" placeholder="KEY=value，每行一个" /></label>
          <label><input v-model="form.providerQuotaConfirmed" type="checkbox" />已在 Dokploy 重新核对 Key 配额与有效期</label>
        </div><div class="grid">
        ...
        <label>Registry 密码<input v-model="form.registryPassword" type="password" autocomplete="off" /><small>GHCR 请使用 Token (classic)，仅授予 read:packages；Fine-grained PAT 不适用。不要填写仓库管理 Token，保存不等于已验证镜像可拉取。</small></label>
      ...
      <p v-if="job.status === 'AWAITING_MERGE'">合并后自动构建镜像；先在 DevPilot 核对版本并人工确认，再到 GitHub Run workflow 选择 operation=release、填写原构建 Run ID 与确认 ID，或运行 GitLab 手动生产任务。首次有效回调和健康检查完成前，不会声称部署已验收。</p>
    ...
    <template v-else>
      <section class="card"><h2>1 · 连接账号与部署平台</h2>
        <label v-if="platform?.providerTokenConfigured" class="consent"><input v-model="form.usePlatformConnection" type="checkbox" :disabled="!!inspection" @change="selectPlatformConnection" />复用初始化中的 Dokploy 授权（Key 仅在服务端使用）</label>
        ...
        <div class="grid">
          ...
          <label>部署平台<select v-model="form.deploymentProvider" :disabled="!!inspection || form.usePlatformConnection"><option>DOKPLOY</option><option>COOLIFY</option></select></label>
          <label>平台地址<input v-model.trim="form.providerBaseUrl" :disabled="!!inspection || form.usePlatformConnection" placeholder="https://deploy.example.com" /></label>
          <label v-if="!form.usePlatformConnection" class="wide">部署平台 API Key<input v-model="form.providerApiToken" :disabled="!!inspection" type="password" autocomplete="off" /><small>应具备读取项目、创建应用和部署权限。仅连接你信任的平台；公网务必使用 HTTPS。</small></label>
        </div>
      ...
      <section v-if="inspection" class="card"><h2>2 · 确认运行参数</h2>
        ...
        <div class="grid">
          ...
          <label>Registry 拉取用户名（私有镜像）<input v-model.trim="form.registryUsername" autocomplete="off" /></label><label>Registry 拉取密码 / Token<input v-model="form.registryPassword" type="password" autocomplete="off" /><small>GHCR 使用 Token (classic) + read:packages，不支持 Fine-grained PAT；请勿复用仓库管理 Token。Dokploy 可自动配置拉取凭据。Coolify 需服务器预先登录 Registry；不会自动公开镜像。保存或登录成功不等于指定镜像可拉取。</small></label>
